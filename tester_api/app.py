...
def consume_messages_from_beginning(topic, timeout=5):
...
def get_cadastro_produtos():
    ...
    try:
        messages = consume_messages_from_beginning(TOPIC_CADASTRO)
...
def get_persisted_products():
    ...
    try:
        messages = consume_messages_from_beginning(TOPIC_PERSISTIDOS)
        logger.info(f"{len(messages)} mensagens lidas do Kafka")
        return jsonify({"status": "success", "messages": messages}), 200
    except Exception as e:
        logger.error(f"Falha ao ler mensagens: {e}")
